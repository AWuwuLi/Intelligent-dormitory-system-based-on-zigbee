...
{ 
  ...
  SampleApp_P2P_DstAddr.addrMode = (afAddrMode_t)AddrBroadcast; //点播 
  SampleApp_P2P_DstAddr.endPoint = SAMPLEAPP_ENDPOINT; 
  SampleApp_P2P_DstAddr.addr.shortAddr =0xFFFF;            //发给协调器
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SAMPLEAPP_SEND_PERIODIC_MSG_EVT )
  {
    ...
    SampleApp_SendPeriodicMessage();//广播模式

    // Setup to send message again in normal period (+ a little jitter)
    osal_start_timerEx( SampleApp_TaskID, SAMPLEAPP_SEND_PERIODIC_MSG_EVT,
        (SAMPLEAPP_SEND_PERIODIC_MSG_TIMEOUT + 4000) );
...
//  osal_memcpy(&strTemp[4], humidity, 3);
   ...
   osal_memcpy(strTemp,"{\"tem4\":", 8); 
   osal_memcpy(&strTemp[8],temp, 2);
   osal_memcpy(&strTemp[10],",\"hum4\":",8);
   osal_memcpy(&strTemp[18],humidity, 2);
   osal_memcpy(&strTemp[20],"}", 1);

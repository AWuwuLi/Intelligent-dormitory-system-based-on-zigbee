/**************************************************************************************************
  ...
  its documentation for any purpose.cst x
...
{ 
  ...
  SampleApp_P2P_DstAddr.addr.shortAddr = 0xFFFF;            //    Э    
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SAMPLEAPP_SEND_PERIODIC_MSG_EVT )
  {
    ...
    SampleApp_SendPeriodicMessage();//? ù???
...
void SampleApp_SendPeriodicMessage( void )
{
  byte i, temp[3], humidity[3], strTemp[30],guangzhao[2];
  Delay_ms(500);
  DHT11();             //  ?  ?  
  Delay_ms(500);
  guang = DATA_PIN;
  ...
  humidity[2] = '\0';
  guangzhao[0] = guang + 0x30;
  guangzhao[1] = '\0';
   //memset(strTemp,0,22);
  //         ????  Э      ?
   osal_memcpy(strTemp,"{\"tem1\":", 8); 
   osal_memcpy(&strTemp[8],temp, 2);
   osal_memcpy(&strTemp[10],",\"hum1\":",8);
   osal_memcpy(&strTemp[18],humidity, 2);
   osal_memcpy(&strTemp[20],",\"gua1\":",8);
   osal_memcpy(&strTemp[28],guangzhao, 1);
   osal_memcpy(&strTemp[29],"}", 1);
  
  //  ?   ?  ?                 ?
  HalUARTWrite(0, "T&H:", 4);
  HalUARTWrite(0, strTemp, 21);
  HalUARTWrite(0, "\n",1);

...
#define WIFIRESET P0_6       // P0.6口控制IGT
#define  devkey "iA8JHWjxuRkaL7oWa8uAkQbSOdk="
#define  devid "586474334"
...
{ 
  ...
  HalUARTWrite(0, "UART0\r\n", 7);
  osal_memset(tmp,0,100);
      ...
      case CMD_SERIAL_MSG:
        ...
        if(flat==1)
        {
           if(strstr((&p->msg[1]),"OK"))
           {
             ...
             HalUARTWrite(0, "AT ERR\r\n", 6); 
        }
         else if(flat==2)
        {
           if(strstr((&p->msg[1]),"OK"))
           {
             // HalUARTWrite(1,CWJAP_C, strlen(CWJAP_C));//连接本地WiFi
              HalUARTWrite(1,"AT+CWJAP=\"CMCC-IOT\",\"linyi1234567890\"\r\n", strlen("AT+CWJAP=\"CMCC-IOT\",\"linyi1234567890\"\r\n"));//连接本地WiFi
              flat=3;
               Delay_ms(500);
           }
          else if(strstr((&p->msg[1]),"ERROR"))
           {
//             ReSetWifi();//重启WiFi
//              Delay_ms(1000);
//              HalUARTWrite(1,"AT\r\n",4);
...
//        else if(flat==7)
//        {
//          //HalUARTWrite(1,strTemp,12);
//          // HalUARTWrite(1, &p->msg[1], p->msg[0]); //把接收到的数据重新发回去                     
//        }
         break;
...
//      HalUARTWrite(0, "\n", 1);         // 回车换行
      ...
      if(flat==7)
      {
        sprintf(HttpData,"POST /devices/%s/datapoints?type=3 HTTP/1.1\r\napi-key:%s\r\nHost:api.heclouds.com\r\n",devid,devkey,pkt->cmd.DataLength);	
        sprintf(niyoudu,"Content-Length:%d\r\n\r\n%s\r\n",pkt->cmd.DataLength,(char *)pkt->cmd.Data);
        HalUARTWrite(0,HttpData,strlen(HttpData)); 
        HalUARTWrite(0,niyoudu,strlen(niyoudu)); 
        Delay_ms(20); 
      }
      break;    
...
//  osal_memcpy(&strTemp[4], humidity, 3);
   ...
   osal_memcpy(strTemp,"{\"temp\":", 8); 
   osal_memcpy(&strTemp[8],temp, 2);
   osal_memcpy(&strTemp[10],",\"hum\":", 7);
   osal_memcpy(&strTemp[17],humidity, 2);
   osal_memcpy(&strTemp[19],"}", 1);
  
  //获得的温湿度通过串口输出到电脑显示
  HalUARTWrite(0, "T&H:", 4);
  HalUARTWrite(0, strTemp, 20);
  HalUARTWrite(0, "\n",1);
